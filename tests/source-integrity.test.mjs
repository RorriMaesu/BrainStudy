...
test("the annotation system is camera-aware, bilateral and collision-managed", async () => {
  ...
  assert.match(scene, /sidePositions:/);
  assert.match(scene, /viewPositions:/);
  assert.match(scene, /posterior:\s*{\s*left:/);
  assert.match(scene, /superior:\s*{\s*left:/);
  assert.match(viewer, /function AnnotationProjector/);
  assert.match(viewer, /type CameraView = CameraPreset \| "free"/);
  assert.match(viewer, /function classifyCamera/);
  assert.match(viewer, /commandId/);
  ...
  assert.match(viewer, /layoutInlineAnnotations/);
  assert.match(viewer, /worldPosition/);
  assert.match(viewer, /annotation-hidden-cue/);
  assert.doesNotMatch(viewer, /const labelEdge|frame\.width - labelEdge/);
  ...
  assert.match(page, /cameraView === "free"/);
  assert.match(page, /cameraCommandId/);
  assert.match(css, /\.annotation-card\.is-left/);
  assert.match(css, /\.annotation-card\.is-right/);
  assert.match(css, /\.annotation-card\.is-inline/);
  assert.match(css, /\.annotation-hidden-cue/);
